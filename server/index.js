...
mongoose
  ...
  .then(() => {
    ...
    deleteNotificationsByUserIdAndUpdateUsers();
  ...
  const deleteNotificationsByUserIdAndUpdateUsers = async () => {
    try {

      const userId = "6536dd3c9535d8ad8ffd3197"
      const users = await User.find();
  
      for (const user of users) {
        const notificationsToRemove = user.notifications.filter(notification => notification.userId === userId);
  
        if (notificationsToRemove.length > 0) {
          user.notifications = user.notifications.filter(notification => notification.userId !== userId);
  
          await user.save();
        }
      }
  
      console.log('Notifications removed and users updated.');
    } catch (error) {
      console.error('Error deleting notifications and updating users:', error);
    }
  };
